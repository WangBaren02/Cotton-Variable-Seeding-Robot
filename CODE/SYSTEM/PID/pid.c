...
void PID_Init(void)
{
	...
	//输出限幅
	PID_R1_Right_Servo.outputLimit = 6;
	...
	//输出限幅
	PID_L1_Left_Servo.outputLimit = 6;
/*
*************************************
* 		电机A转速闭环PID参数初始化			
*************************************
...
*/	
	...
	//微分
	PID_9250_Forward.kd = 0.6;
	//输出限幅
	PID_9250_Forward.outputLimit = 6;
	
	PID_9250_Forward_motor.desired = 0;
	//比例
	PID_9250_Forward_motor.kp = 0;
	//积分
	...
	//微分
	PID_9250_Forward_motor.kd = 0;
	//输出限幅
	PID_9250_Forward_motor.outputLimit = 500*n;
/*
*************************************
* 		右行时9250PID参数初始化
*************************************
*/
	...
	//微分
	PID_9250_Right.kd = 59;//82
	//输出限幅
	PID_9250_Right.outputLimit = 800*n;
	...
	//输出限幅
	PID_9250_Right_slow.outputLimit = 800*n;
	
	PID_9250_Back_last_motor.desired = 0;
	//比例
	PID_9250_Back_last_motor.kp = 1000;
	//积分
	...
	//微分
	PID_9250_Back_last_motor.kd = 0;
	//输出限幅
	PID_9250_Back_last_motor.outputLimit = 2000*n;
/*
*************************************
* 		左行时9250PID参数初始化			
*************************************
*/	
	...
	PID_9250_Left.kd = 36;
	//输出限幅
	PID_9250_Left.outputLimit = 800*n;
...
/****************换垄****************/
	...
	//微分
	PID_9250_Back.kd = 0.556;
	//输出限幅
	PID_9250_Back.outputLimit = 6;
/****************出垄****************/
	PID_9250_Back_last.desired = 0;
	//比例
	PID_9250_Back_last.kp = 1.824;
	//积分
	...
	//微分
	PID_9250_Back_last.kd = 0.556;
	//输出限幅
	PID_9250_Back_last.outputLimit = 6;
/*
*************************************
* 		靠左前行时pid初始化			超声波
*************************************
*/
	...
	//微分
	PID_L1_For_Servo.kd = 0.1;
	//输出限幅
	PID_L1_For_Servo.outputLimit = 4;
	
	PID_L2_For_Servo.desired = L2_F;
	//比例
	PID_L2_For_Servo.kp = 0.2;
	//积分
	...
	//微分
	PID_L2_For_Servo.kd = 0.1;
	//输出限幅
	PID_L2_For_Servo.outputLimit = 4;
/*
*************************************
* 		靠左后退时pid初始化		  超声波
*************************************
*/
  PID_L1_Back_Servo.desired = L1;
	...
	//微分
	PID_L1_Back_Servo.kd = 0.8;
	//输出限幅
	PID_L1_Back_Servo.outputLimit = 5;           //1

	PID_L2_Back_Servo.desired = L2;
	//比例
	PID_L2_Back_Servo.kp = 0.4;
	//积分
	...
	//微分
	PID_L2_Back_Servo.kd = 0.8;
	//输出限幅
	PID_L2_Back_Servo.outputLimit = 5;           //2
...
*/
	...
	//输出限幅
	PID_R1_For_Servo.outputLimit = 4;
...
//	PID_F2_Left_Servo.outputLimit = 10;	
	...
	//比例
	PID_F1_Left_Servo.kp = 0.17;
	//积分
	PID_F1_Left_Servo.ki = 0;
	PID_F1_Left_Servo.integ_sep = 15;
	PID_F1_Left_Servo.dt = 1;
	PID_F1_Left_Servo.iLimit = 0.2;
	//微分
	PID_F1_Left_Servo.kd = 0.17;
	//输出限幅
	PID_F1_Left_Servo.outputLimit = 4;//4
	
	
	PID_F2_Left_Servo.desired = F2;
	//比例
	PID_F2_Left_Servo.kp = 0.17;
	//积分
	PID_F2_Left_Servo.ki = 0;
	PID_F2_Left_Servo.integ_sep = 15;
	PID_F2_Left_Servo.dt = 1;
	PID_F2_Left_Servo.iLimit = 0.2;
	//微分
	PID_F2_Left_Servo.kd = 0.17;	//0.2
	//输出限幅
	PID_F2_Left_Servo.outputLimit = 4;
/*
*************************************
* 		F12超声波控制舵机右行pid			
*************************************
*/
  PID_F1_Right_Servo.desired = F1;
	//比例
	PID_F1_Right_Servo.kp = 0.17;//0.1
	//积分
	PID_F1_Right_Servo.ki = 0;
	PID_F1_Right_Servo.integ_sep = 15;
	PID_F1_Right_Servo.dt = 0;
	PID_F1_Right_Servo.iLimit = 0.2;//0.2
	//微分
	PID_F1_Right_Servo.kd = 0.08;//0.5
	//输出限幅
	PID_F1_Right_Servo.outputLimit = 4;//5


	PID_F2_Right_Servo.desired = F2;
	//比例
	PID_F2_Right_Servo.kp = 0.17;
	//积分
	PID_F2_Right_Servo.ki = 0;
	PID_F2_Right_Servo.integ_sep = 15;
	PID_F2_Right_Servo.dt = 0;
	PID_F2_Right_Servo.iLimit = 0.2;
	//微分
	PID_F2_Right_Servo.kd = 0.08;
	//输出限幅
	PID_F2_Right_Servo.outputLimit = 4;
/*
*************************************
* 		F1超声波控制电机左行pid			
*************************************
...
void Control_L1_For_Servo(float L1_dis)				//用左侧第一个超声波的值控制前面两个舵机
{
...
void Control_L2_For_Servo(float L2_dis)				//用左侧第二个超声波的值控制前面后个舵机
{
...
void Control_L1_Back_Servo(float L1_dis)				//用左侧第二个超声波的值控制前面两个舵机
{
...
void Control_L2_Back_Servo(float L2_dis)				//用左侧第二个超声波的值控制前面两个舵机
{
...
void Control_L1_Backlast_Servo(float L1_dis)				//用左侧第二个超声波的值控制前面两个舵机,出垄
{
...
void Control_L2_Backlast_Servo(float L2_dis)				//用左侧第二个超声波的值控制前面两个舵机，出垄
{
...
void Control_R1_For_Servo(float R1_dis)				//用左侧第一个超声波的值控制前面两个舵机
{
...
void Control_R2_Back_Servo(float R2_dis)				//用左侧第二个超声波的值控制前面两个舵机
{
...
void Control_R1_Back_Servo(float R1_dis)				//用左侧第二个超声波的值控制前面两个舵机
{
...
void Control_F1_Left_Servo(float F1_dis)				//用前面第一个超声波值控制左边两个舵机
{
	PID_F1_Left_Servo.error =PID_F1_Left_Servo.desired - F1_dis;
	Pid_Update(&PID_F1_Left_Servo, PID_F1_Left_Servo.error);
	Servo2_Increase = (-PID_F1_Left_Servo.out);	//-
	Servo4_Increase = (-PID_F1_Left_Servo.out);
...
void Control_F2_Left_Servo(float F2_dis)				//用前面第一个超声波值控制左边两个舵机
{
	PID_F2_Left_Servo.error =PID_F2_Left_Servo.desired - F2_dis;
	Pid_Update(&PID_F2_Left_Servo, PID_F2_Left_Servo.error);
	Servo1_Increase = (-PID_F2_Left_Servo.out);
	Servo3_Increase = (-PID_F2_Left_Servo.out);
...
void Control_F1_Right_Servo(float F1_dis)			//用前面超声波值控制
{
	PID_F1_Right_Servo.error = PID_F1_Right_Servo.desired - F1_dis;
	Pid_Update(&PID_F1_Right_Servo, PID_F1_Right_Servo.error);
	Servo2_Increase = PID_F1_Right_Servo.out;
	Servo4_Increase = PID_F1_Right_Servo.out;
...
void Control_F2_Right_Servo(float F2_dis)			//用前面超声波值控制
{
	PID_F2_Right_Servo.error = PID_F2_Right_Servo.desired - F2_dis;
	Pid_Update(&PID_F2_Right_Servo, PID_F2_Right_Servo.error);
	Servo1_Increase = PID_F2_Right_Servo.out;
	Servo3_Increase = PID_F2_Right_Servo.out;
...
void Control_R1_Right_Servo(float D_dis,u8 step)			//用前面第二个超声波值控制右边两个舵机
{
...
void Control_L1_Left_Servo(float D_dis,u8 step)			//用前面第二个超声波值控制右边两个舵机
{
...
void Contrl_9250_Forwardl(float angle)					//用yaw角控制车直走
{
  ...
  Servo3_Increase = PID_9250_Forward.out;
	...
	Servo5_Increase = PID_9250_Forward.out;
	Servo6_Increase = PID_9250_Forward.out; 
  Servo7_Increase = PID_9250_Forward.out;
	Servo8_Increase = PID_9250_Forward.out; 
}
...
void Contrl_9250_Back(float angle)					//用yaw角控制车后退
{
...
void Contrl_9250_Back_last(float angle)					//用yaw角控制车后退
{
...
void Contrl_9250_Back_last_motor(float angle)					//用yaw角控制车后退
{
	PID_9250_Back_last_motor.error = angle - PID_9250_Right.desired;
